...
class SliderConfig(InfoComponentConfig[float | None, gr.Slider]):
    ...
    def clean_strength(cls, visible: bool) -> SliderConfig:
        ...
        return cls(
            label="Сила очистки",
            info=(
                "Чем выше значение, тем сильнее шумоподавление. Слишком высокая "
                "очистка может сделать голос более сжатым и менее живым."
            ),
            value=0.7,
            minimum=0.0,
            maximum=1.0,
    ...
    def cpu_cores(cls) -> SliderConfig:
        ...
        return cls(
            label="Ядра CPU",
            info="Сколько ядер процессора использовать для параллельной обработки.",
            value=None,
            minimum=1,
            maximum=1,
            step=1,
...
class DropdownConfig(InfoComponentConfig[BaseDropdownValue, gr.Dropdown]):
    ...
    def hardware_acceleration(cls) -> DropdownConfig:
        ...
        return cls(
            ...
            info=(
                "Какое ускорение использовать. Автоматический режим выберет первую "
                "доступную GPU, а если её нет, переключится на CPU."
            ),
    ...
    def song_dir(cls) -> DropdownConfig:
        ...
        return cls(
            label="Папка трека",
            info=(
                "Папка с промежуточными дорожками. После загрузки нового трека она "
    ...
    def gpu(cls) -> DropdownConfig:
        ...
        return cls(
            label="GPU",
            info="Одна или несколько видеокарт для аппаратного ускорения.",
            value=None,
            multiselect=True,
            visible=False,
            exclude_value=True,
