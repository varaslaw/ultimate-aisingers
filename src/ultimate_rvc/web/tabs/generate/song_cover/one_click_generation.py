...
REVERB_PRESETS = {
    "Почти сухо": (0.05, 0.06, 0.94, 0.85),
    "Естественно": (0.15, 0.18, 0.82, 0.7),
    "Мягкая комната": (0.28, 0.24, 0.76, 0.62),
    "Большой зал": (0.45, 0.32, 0.68, 0.55),
}
...
def render(total_config: TotalConfig, cookiefile: str | None = None) -> None:
    ...
    with gr.Tab("В один клик"):
        ...
        sound_template = gr.Radio(
            ...
            info=(
                "Ваш мягкий пресет: FCPE и пониженные значения индекса, RMS, "
                "защиты и обработки. Остальные варианты сохраняют более "
                "выраженный характер модели."
            ),
        )
    ...
    with gr.Accordion("Сведение", open=False):
        ...
        with gr.Accordion("Реверб на сконвертированном вокале", open=False):
            reverb_preset = gr.Dropdown(
                label="Пресет реверберации",
                value="Естественно",
                choices=list(REVERB_PRESETS),
            )
            with gr.Row():
...
def _select_reverb_preset(preset: str) -> list[gr.Slider]:
    room_size, wet_level, dry_level, damping = REVERB_PRESETS[preset]
    return [
        gr.Slider(value=room_size),
        gr.Slider(value=wet_level),
        gr.Slider(value=dry_level),
        gr.Slider(value=damping),
    ]
