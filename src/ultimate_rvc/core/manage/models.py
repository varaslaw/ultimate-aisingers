...
def _extract_voice_model(
    ...
    try:
        ...
        for path in extraction_path.rglob("*"):
            ...
            if suffix in file_candidates and path.stat().st_size > MODEL_FILE_MIN_SIZE:
                file_candidates[suffix].append(path)
        file_path_map = {
            suffix: max(paths, key=lambda path: path.stat().st_size)
            for suffix, paths in file_candidates.items()
            if paths
        }
        ...
        for file_path in file_path_map.values():
            destination = extraction_path / file_path.name
...
def _as_uploaded_path(file: StrPath | object) -> Path:
...
def _get_download_filename(url: str, name: str) -> str:
    parsed = urlparse(url)
    filename = Path(parsed.path).name
    if filename and "." in filename:
        return filename
    safe_name = re.sub(r"[^\w .-]", "_", name).strip() or "voice_model"
...
def _download_google_drive(url: str, destination: Path) -> bool:
...
def _download_yandex_disk(url: str, destination: Path) -> bool:
    ...
    response.raise_for_status()
    _download_direct(response.json()["href"], destination)
    return True


def _download_mega(url: str, destination: Path) -> bool:
    if "mega.nz" not in url and "mega.co.nz" not in url:
        return False
    from mega import Mega  # noqa: PLC0415
...
def _download_model_archive(url: str, destination: Path) -> None:
    if _download_google_drive(url, destination):
        return
    if _download_yandex_disk(url, destination):
        return
    if _download_mega(url, destination):
        return
    _download_direct(_normalize_model_url(url), destination)
...
def download_voice_model(url: str, name: str) -> None:
    ...
    zip_path = Path(_get_download_filename(url, name))

    display_progress("[~] Downloading voice model ...")
    _download_model_archive(url, zip_path)

    display_progress("[~] Extracting zip file...")
    _extract_voice_model(zip_path, extraction_path, remove_zip=True)
...
def upload_voice_model(
    ...
    sorted_file_paths = sorted(
        [_as_uploaded_path(f) for f in files],
    ...
    match sorted_file_paths:
        ...
        case [index_path, pth_path]:
            if (
                index_path.suffix.lower() == ".index"
                and pth_path.suffix.lower() == ".pth"
            ):
...
def delete_all_models() -> None:
    """Delete all models used by the generation interface."""
    display_progress("[~] Deleting all voice models ...", 0.0)
    delete_all_voice_models()
    display_progress("[~] Deleting all custom embedder models ...", 0.5)
